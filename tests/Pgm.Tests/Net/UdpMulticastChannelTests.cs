...
public sealed class UdpMulticastChannelTests
{
    ...
    public async Task SendReceive_LoopbackMulticast_RoundTrips()
    {
        ...
        {
            // Multicast loopback delivery is not guaranteed on all hosts (e.g. CI); the send path is still exercised.
        }
        catch (System.Net.Sockets.SocketException)
        {
            // Multicast is unavailable on this host; the construction and send call paths are still exercised.
        }
    }
    ...
    {
        var channel = new UdpMulticastChannel(IPAddress.Parse("239.192.0.8"), 3058, IPAddress.Loopback);
        await channel.DisposeAsync();
        await channel.DisposeAsync();

        await Assert.That(async () => await channel.ReceiveAsync(new byte[16])).Throws<ObjectDisposedException>();
    }
}
